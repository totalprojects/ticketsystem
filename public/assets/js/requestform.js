...
function loadTcodes(dependencies = []) {
  pickerTreeRender([]);
  $('#modules_tcodes_block').append(
    "<h3 class='ldr_tc badge badge-warning p-1 m-1'><i class='fas fa-spinner fa-spin'></i> Loading...</h3>"
  );
  ...
  switch (step) {
    ...
    case 3:
      ...
      if (purchase.length == 0 && sales.length == 0) {
        toastr.error(
          'Either purchase / sales input must be filled to continue'
        );
        flag = false;
      }
...
$('#finalSubmit1').on('click', (e) => {
  ...
  var formData = $('#msform').serializeArray();
  var finalArray = [];
  var isByRM = $('#isByRM').val();

  if (isByRM !== undefined) {
    let userID = $('#user_id').val();
    formData.push({ addedByRM: true, userID: userID });
  }
  $.each(data, (i) => {
    finalArray[i] = {
      moduleset: data[i].addional,
