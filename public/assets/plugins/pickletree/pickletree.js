...
class PickleTree {
  ...
  constructor(obj) {
    ...
    this.main_container = document.getElementById(
      this.config.key + '_div_pickletree'
    );
    //start events
    this.staticEvents();
  }
  ...
  staticEvents() {
    ...
    if (this.config.drag) {
      ...
      this.main_container.addEventListener('dragstart', async (e) => {
        ...
        //container
        this.invalid_area.container = document.getElementById(
          this.target + 'node_' + e.target.id.split('node_')[1]
        );
        this.invalid_area.top =
          this.invalid_area.container.getBoundingClientRect().top;
        this.invalid_area.left =
          this.invalid_area.container.getBoundingClientRect().left;
        this.invalid_area.right =
          this.invalid_area.left + this.invalid_area.container.offsetWidth;
        this.invalid_area.bottom =
          this.invalid_area.top + this.invalid_area.container.offsetHeight;
        setTimeout(() => {
        ...
        if (this.dragCallback) {
          this.dragCallback(
            this.nodeList[parseInt(e.target.id.split('node_')[1])]
          );
        }
      });
      ...
      this.main_container.addEventListener('dragenter', (e) => {
        ...
        try {
          ...
          if (
            target.top > this.invalid_area.top &&
            target.top < this.invalid_area.bottom &&
            target.left > this.invalid_area.left &&
            target.left < this.invalid_area.right
          ) {
  ...
  build(c_config) {
    ...
    document.getElementById(this.target).innerHTML =
      '<div id="' +
      this.config.key +
      '_div_pickletree"><ul id="' +
      this.config.key +
      '_tree_picklemain"></ul></div>';
  ...
  updateNode(node) {
    ...
    if (node.old_parent !== undefined && node.old_parent.id !== 0) {
      this.nodeList[node.old_parent.value].childs = this.nodeList[
        node.old_parent.value
      ].childs.filter((x) => {
        return x !== node.id;
      });
      //if child count is 0 then remove minus icon
      if (this.nodeList[node.old_parent.value].childs.length === 0) {
        document.getElementById('i_' + node.old_parent.id).style.display =
          'none';
      }
    }
  ...
  drawNode(node) {
    ...
    if (typeof this.rowCreateCallback == 'function')
      this.rowCreateCallback(node);
  }

  setNodeEvents(node) {
    //toggle event for node
    document
      .getElementById('a_toggle_' + node.id)
      ...
      let set = (list) => {
        for (let i = 0; i < list.length; i++) {
          this.createNode({
            ...
            n_checkStatus:
              typeof list[i].n_checkStatus === 'undefined'
                ? false
                : list[i].n_checkStatus,
          });
          if (list[i].Child) {
            set(list[i].Child);
          }
  ...
  drawMenu(obj) {
    ...
    if (document.getElementById('div_menu_' + obj.node.id) === null) {
      ...
      for (let i = 0; i < obj.node.elements.length; i++) {
        ...
        icon =
          obj.node.elements[i].icon.trim().length > 0
            ? '<i class="' + obj.node.elements[i].icon.trim() + '"></i>'
            : '';
        ...
        span_item.addEventListener('click', (e) => {
          obj.node.elements[i].onClick(
            this.getNode(e.target.getAttribute('data-node').split('node_')[1])
          );
